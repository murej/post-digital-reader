...
//@codekit-append "jquery.validate.js"
...
/** ********* ****************************************************************/
					...
					$(window).scrollTo(
						...
						{ onAfter: function() {
								...
								if(target.indexOf("paragraph") !== -1) {
									...
									$(window).stop().scrollTo( ($("div#cover").height()*2), 1, { onAfter: function() {
											
											$.waypoints("refresh");
											$(window).scrollTo( $(target), 1000, {easing:'easeInOutQuint'} );
										}
									}); 
								}
									...
									if( $(target).length !== 0 ) {
										...
										$(window).stop().scrollTo( ($("div#cover").height()*2), 1, { onAfter: function() {
												...
												$.waypoints("refresh");
												$(window).scrollTo( $(target), 250, {easing:'easeInOutQuint'} );
											}
										}); 
									}
								}
	...
	function resizeRibbon() {
		...
		$("#collector span.ribbon").css("border-left-width",(ribbonWidth/parseFloat($("#collector").css("font-size"))/2)+"em");
		$("#collector span.ribbon").css("border-right-width",(ribbonWidth/parseFloat($("#collector").css("font-size"))/2)+"em");
	}
	...
	function updateCollector(num) {
		...
		if( num !== 0 ) {
			...
			$("li.viewing a").show();
			$("li.viewing span.disabled").hide();
		}
		else if( getUrlQueryParameter("edition") !== "-1" ) {

			$("li.viewing a").hide();
			$("li.viewing span.disabled").show();
		}

		if( num === 0 && getUrlQueryParameter("edition") === "-1" ) {
			window.location.reload();
		}


/*
		if( $("li.paragraph div.paragraph-num a.selected").length !== 0 ) {

			var insert = $("li.paragraph div.paragraph-num a.selected").length+"<span class='mini'>/ "+num+"</span>";
		}
		else {
...
*/
	...
	function animateIllustrations() {
		...
		//console.log("Vol: ["+minVolume+","+maxVolume+"]");
		//console.log("Act: ["+minActivity+","+maxActivity+"]");
		...
		$("li.chapter").each(function() {
			...
			// APPLY EFFECT WITH NORMALISED VALUES HERE !!!!




			//$(this).children("a").children("div").fadeTo(0, map(volume, minVolume, maxVolume, 0, 1) );
			//$(this).children("a").children("div.title").css("color", "rgba(0,0,0,"+map(volume, minVolume, maxVolume, 0, 1)+")" );
...
/** ************* ************************************************************/
	...
	Hyphenator.run();
	scrollToHash();
	$("form#publish-form").validate();
	$("div#writer form").validate();
	$("div#add-reference form").validate();
	/**************/
	...
	$("li.introduction span.what-is").keyup( function() {

		if(event.keyCode === 13) {

			var input = $(this);

			ajax_object.definition = input.text();

			var data = {
				action:			'define_this',
				nonce:			ajax_object.nonce,
				definition:		ajax_object.definition
			};

			$.post(ajax_object.ajax_url, data);

			input.parent().removeClass("error").addClass("success");
			input.removeAttr('contenteditable').blur();
		}
	});
	...
	$("li#collection-info form#publish-form").keyup( function() {

		var input = $(this).children("input[name=editionTitle]");

		ajax_object.editionTitle = input.val();

		var data = {
			action:			'validate_edition',
			nonce:			ajax_object.nonce,
			editionTitle:	ajax_object.editionTitle
		};

		$.post(ajax_object.ajax_url, data, function(response) {

			// if title matched existing
			if( response === 'true' ) {

				// throw an error
				input.addClass("match-error");
			}
			// if title new
			else {

				// remove error
				input.removeClass("match-error");
			}
		});
	});

	$("li#collection-info form#publish-form").submit( function(e) {

		if( $(this).children(".match-error").length !== 0 ) {
	...
	$("a[href=#collection-info]").click(function(e) {
		
		if( $("body").hasClass("home") ) {
		
			e.preventDefault();
			$(window).scrollTo( $("li#collection-info"), 1000 );
		}
	});
	...
	$("ul#nav li.viewing span.disabled").hover(
		function() {
			$("div.paragraph-num").addClass("hover");
		},
		function() {
	...
	$(window).keyup(function(e) {
		...
		if(e.keyCode === 27 && $("body").hasClass("home") && ($("body").hasClass("edition-select-mode") === false) ) {
	...
	// EDITION SELECT CANCEL
	$(window).keyup(function(e) {

		if(e.keyCode === 27 && $("body").hasClass("edition-select-mode")) {
